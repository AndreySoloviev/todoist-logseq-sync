...
(() => {
  ...
  // src/index.ts
  var import_libs2 = __toESM(require_lsplugin_user(), 1);
  ...
  var TodoistClient = class {
    ...
    async getInboxTasks(inboxProjectId) {
      let projectId = inboxProjectId;
      if (!projectId) {
        projectId = await this.getInboxProjectId();
      }
      const tasks = await this.request(`/tasks?project_id=${encodeURIComponent(projectId)}`);
      return tasks;
    }
  ...
  function startScheduler(settings) {
    stopScheduler();
    const intervalMs = Math.max(1, settings.intervalMinutes) * 6e4;
  ...
  logseq.ready(async () => {
    registerUI();
    logseq.App.onSettingsChanged((newSettings) => onSettingsChanged(newSettings));
    const settings = logseq.settings ?? defaultSettings;
    startScheduler(settings);
    logseq.provideModel({
